...
const CSVUpload: React.FC<CSVUploadProps> = ({ onDataParsed, onError }) => {
    ...
    (acceptedFiles: File[]) => {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          const mappedData = typedData.map((row) => ({
            ehrId: row['EHR ID'] || '',
  ...
  const { getRootProps, getInputProps, isDragActive: dropzoneIsDragActive } =
    useDropzone({
      onDrop,
      accept: {
        "text/csv": [".csv"],
      ...
      {isLoading ? (
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            File should include: EHR ID, Patient Name, Email, Phone, Referring
            Provider
          </p>
        </div>
      )}
